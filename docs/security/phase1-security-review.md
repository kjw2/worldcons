...
- CSP report는 `site_events`에 `security_event`로 저장하며, directive, blocked URI, document URI, source file, sample 등 표시 가능한 범위의 메타데이터만 남깁니다. URL/query/API key/token/password 형태의 민감값은 저장 전에 redaction 처리합니다.
