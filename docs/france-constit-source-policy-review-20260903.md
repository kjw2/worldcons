...
On 2026-09-16 the WorldCons owner explicitly approved this source policy, scoped only to the France 2010-2024 `QPC`/`DC` tranches. Spain and the Germany 1998-2023 expansion are **not** covered by this approval. Migration `20260916090000_constitutional_case_france_policy_approval.sql` inserts the immutable `source_corpus_policies` row `fr-conseil-constitutionnel` / `france-dila-constit-2026-09-v1`. On the same date the owner approved the two M5-B2.3 exceptions E1 (2022 DC Conseil omission `2022847DC`) and E2 (2022 QPC DILA canonicalization pair `20225813AN_QPC`); the additive successor row `france-dila-constit-2026-09-v2` (migration `20260916100000`) supersedes v1 without modifying it. **Neither v2 migration is applied to production, and no 2022 backfill or snapshot has been run.**
...
- M5-B3.1 completed the 2023 QPC/DC private-shadow expansion (QPC 45/45, DC 15/15).
- M5-B2.2 implemented the ordered increment overlay and identified the two 2022 source-shape anomalies. M5-B2.3 was **owner-approved for both E1 and E2** on 2026-09-16 and implemented as additive v2 migrations `20260916100000`/`20260916101000`/`20260916102000`. **Production migration is NOT applied and no 2022 backfill/snapshot was run; France stays 4/30 production-complete.**
- The default France history execution flag remains off after those scoped commands. Multi-year expansion remains a separate explicitly controlled stage, shipped newest-to-oldest, and must not proceed to 2021 while 2022 is unresolved.
