...
`public_article_projection_p3` exposes exactly the immutable version selected by a current `published` publication. Visibility never re-evaluates live legacy or P2 fields: processing, review, attention, queue, and legacy policy changes after publication cannot hide or restore the row. Only an explicit publication transition changes visibility. The projection carries article tags in the same bounded query and exposes the stored search vector and embedding. `public_tag_projection_p3`, `public_jurisdiction_article_counts_p3`, and `match_public_article_versions_p3` use that same identity set. There is no application-side publication predicate when P3 reads are enabled.
