...
WorldCons uses one vector space only: `gemini-embedding-001`, 1536 dimensions, L2-normalized. Documents use `RETRIEVAL_DOCUMENT`; provider search queries use `RETRIEVAL_QUERY`. The application rejects any other `EMBEDDING_PROVIDER`.

## Safe production order

1. Set the Vercel Production secret `GEMINI_API_KEY`, deploy the application with `SEMANTIC_SEARCH_ENABLED=false`, and verify semantic/hybrid requests report an explicit full-text fallback.
...
6. Change `SEMANTIC_SEARCH_ENABLED=true`, deploy the Vercel application, then canary exact-case, full-text, semantic, and hybrid provider searches. Exact-case preflight must remain embedding-free.

## Rollback

Set `SEMANTIC_SEARCH_ENABLED=false` and redeploy the Vercel application. This restores explicit full-text fallback without deleting vectors, changing immutable article versions, or reverting a database migration. Do not switch query generation back to OpenAI: newly written and backfilled documents are Gemini vectors.
