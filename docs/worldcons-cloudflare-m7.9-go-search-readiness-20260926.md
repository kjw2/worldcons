...
**LEXICAL POLICY/HOLDOUT PASS; DEPLOYED RUNTIME PENDING; NO GO-SEARCH.**
...
The remaining M7 condition is a bearer-authenticated run through the actually
deployed `worldcons-search-canary` Worker. The older p50/p95 161/386 ms evidence
was measured through a local runtime with remote bindings and is not reused as
a deployed-Worker SLO.
...
- deployed version: `720d2c2a-e2fc-4f05-9aae-fd425b7a392a`, 100%
...
| deployed bearer-path runtime | M7.9 runtime report | PENDING |

`GO-SEARCH` may be recorded only when the M7.9 runtime report is `state=pass`
and the final gate rechecks all rows above. Even then, this is readiness only:
it does not switch `SearchRepository`, production authority, DNS, routes or
traffic. Cutover remains a separate reversible step.
