...
> **Backfill invariant:** France historical acquisition is not gated by Gemini quota or by completion of earlier public enrichment. A closed, fetched, normalized, verified, reconciled QPC/DC tranche is a corpus-backfill completion; Gemini translation/summary and public Catalog/P3 release are tracked separately and may proceed gradually. Canonical policy: `docs/worldcons-historical-backfill-operating-principles-20260916.md`. Exact targets and the per-year table: `docs/worldcons-historical-backfill-target-plan-20260917.md` (France class-A target 1,280 = 1,005 QPC + 275 DC).
...
The source-policy evidence and proposed immutable row are in [france-constit-source-policy-review-20260903.md](./france-constit-source-policy-review-20260903.md). On 2026-09-16 the WorldCons owner approved that policy for the 2010-2024 `QPC`/`DC` tranches only. Migration `20260916090000_constitutional_case_france_policy_approval.sql` inserts the immutable policy row `fr-conseil-constitutionnel` / `france-dila-constit-2026-09-v1` (review due `2027-03-15`). On the same date the owner also approved BOTH v2 exceptions and the additive successor `france-dila-constit-2026-09-v2`; v1 stays immutable and continues to bind the closed 2024/2023 snapshots. On 2026-09-17 the owner directed continuation through 2010 and approved the additive successor `france-dila-constit-2026-09-v3`, which adds exactly six owner-approved 2017 QPC DILA-omission tuples (see below). v2 stays immutable and continues to bind the closed snapshots that used it. Spain and the Germany 1998-2023 expansion are not approved by this decision.
...
Policy v2 adds exactly two owner-approved provenance shapes, and only under the v2 policy version:

- the E2 canonical item additionally records `dila.retirement` with `retiredDilaId`, `canonicalDilaId`, `conseilRecordId`, `basis`, `verifiedAt`, and `corroborationRef`; the retired DILA ID never becomes a second inventory item;
- the E1 Conseil-provider item records `provider: "conseil"`, `reasonCode: "dila_omission_verified_absent"`, the exact `authorityUrl`, a `conseil` identity block (ECLI/decision number/date/JORF/NOR), and a per-discover `dilaLookup` absence proof. It must never carry a fabricated DILA ID, archive member path, or DILA stock attribution; its `license.attribution` is `Conseil constitutionnel`.
...
The worker uses the v3 upsert, v2 close, and v2 claim RPCs. Production `service_role` execution is revoked from the corresponding v1/v2 upsert RPCs, preventing an application path from omitting provenance or accepting the E1 shape under any policy other than the exact v2 version. The provenance is copied into `metadata.sourceInventory` for bounded fetch replay and normalization, so the Catalog source revision receives the same immutable evidence. Direct table writes remain unavailable to the worker.
...
M5-B2.3 drafted a proposal for that decision: [worldcons-m5b23-france-2022-source-policy-v2-proposal-20260916.md](./worldcons-m5b23-france-2022-source-policy-v2-proposal-20260916.md). It separates the 2022 DC official-source omission (E1, exact one-case Conseil fallback `2022847DC`, stable key `constit:conseil-omission:2022847dc`, reason code `dila_omission_verified_absent`) from the 2022 QPC DILA canonicalization pair (E2, canonical `CONSTEXT000047955984`, retired `CONSTEXT000046216504`, basis `matches_current_conseil_title_and_ecli`), and explains that DILA itself carries no supersession signal so both require external corroboration and owner approval.

**On 2026-09-16 the WorldCons owner explicitly approved BOTH E1 and E2**, and the additive successor `france-dila-constit-2026-09-v2` (superseding v1) was implemented as new timestamp migrations `20260916100000` (policy row), `20260916101000` (strict `source_inventory_item_upsert_v3` E1 branch), and `20260916102000` (France attribution validator/guard v2). v1 stays immutable and continues to bind the closed 2024/2023 snapshots. **These migrations are NOT applied to production and no 2022 production backfill or snapshot has been run.** The default flag still yields `case_backfill.france_history_disabled`, Catalog/Gemini stay off, 2024/2023 snapshots stay immutable, and the rollout remains stopped at 2022 with France QPC/DC still **4/30 production-complete**. France production-complete stays 4/30 until an actual 2022 private-shadow succeeds.

M5-B3.2 subsequently applied all three v2 migrations and completed the 2022 production private-shadow wave. QPC snapshot `1273e11d-e754-4fcc-809a-e23d7de2a231` sealed **67/67** and verified all 67 items with exactly one E2 retirement; DC snapshot `de5e7ff3-75ff-4458-b2d7-44c1268618a3` sealed **13/13** and verified all 13 items with exactly one E1 Conseil-provider fallback. All ten P1 runs succeeded, item errors/retryable/terminal failures/active claims were zero, Catalog publications and publish commands remained zero, Gemini/AI remained off, and the 2024/2023 manifests were unchanged. France QPC/DC is now **6/30 production-complete**. See `worldcons-m5b32-france-2022-private-shadow-completion-20260916.md`.

Controller hardening then made the exception evidence replayable and fail-closed: E1 re-scans every raw DILA XML member in the selected stock plus all ordered increments for both `2022847DC` and `CSCL2237744S`, and requires the current official Conseil detail page to match the frozen ECLI/JORF/title evidence; E2 requires both the exact two-DILA-ID pair and current Conseil detail title/ECLI matching the frozen reviewed values before choosing `CONSTEXT000047955984`. The post-hardening live read-only verifier now passes **2022 QPC 67/67** with exactly one E2 application and **2022 DC 13/13** with exactly one E1 application. Both runs re-used the approved base stock SHA-256 `67270556060b481ec139f21436244af913cccd3eb6e074c65d6600f48596f627`, 21 ordered increments, and chain hash `556e75db2021b0396125bb880555f831c79da554a1a31ed8ba2201114529562d` without opening a snapshot or writing production data.

M5-B4.1 completed the 2021 wave. The first pass stopped 2021 QPC fail-closed: the official Conseil QPC facet reports 75 rows but two official titles render the type token glued to the next lowercase word (`Décision n° 2021-897 QPCdu 16 avril 2021`, `Décision n° 2021-911/919 QPCdu 4 juin 2021`), which the then-current `\bQPC\b` title gate dropped (`expected 75, discovered 73`); the same glued rendering also appears in the detail-page `og:title`/`h1.title`. No 2021 QPC snapshot/run/item/claim was created on that pass and the rollout did not skip to 2020. A reviewed, glue-tolerant remedy then centralized the type check in the exported `franceConseilTitleMatchesType()` in `lib/crawlee/france-conseil-inventory.ts` and reused it from the France verify strategy in `lib/backfill/source-strategies.ts`, so discovery and verify share one exact rule (normal word boundary, or the exact marker followed by a lowercase letter; `QPC360`, `QPCDU`, cross-facet, and other glued tokens still fail). Regression tests cover both directions. The 2021 DC production private-shadow snapshot `a604f20a-433b-4b48-b11b-03168cfccb31` sealed **21/21** verified items (manifest `26f1d80c29b4ffc7b4505625489f36651721fffc9d0488e53de9e125a510eff4`) and the 2021 QPC snapshot `b41c2224-58a8-4323-8f63-09ee198eab9c` sealed **75/75** verified items (manifest `959c26b6b168189b7630424b150a8ba29083dc966cf54032d2691bf2a5e53456`), both with zero exceptions, zero errors, zero active claims, zero Catalog publications, and Gemini off. France QPC/DC corpus completion advanced to **8/30** at that point; the frontier at that time was 2020, which was completed in the next wave below. See [worldcons-m5b41-france-2021-dc-completion-qpc-blocker-20260916.md](./worldcons-m5b41-france-2021-dc-completion-qpc-blocker-20260916.md).

The 2020 wave then closed the newest five years. The 2020 DC production private-shadow snapshot `4c2e4573-a3e5-4597-be73-8e53f3c09d94` sealed the exact 2020 DC target **16/16** and the 2020 QPC snapshot `c8ee1103-9ba9-44a6-9ab2-1468bc1be826` sealed the exact 2020 QPC target **46/46**; both were fully fetched, normalized, verified, and reconciled with zero active claims, zero retry/terminal failures, zero Catalog publications, and Gemini off. Closed manifests: QPC `aac15a6045295a1c1aa73f08965d106c087f6560cc4b93f56c3d457d38d63640`, DC `dec83837f788625f5c5052ae7b2bcac138523eb5aeb7538a3ae1c9d0dd760045`. France QPC/DC corpus completion is therefore **10/30** (352/1,280 items, 2020-2024) and the next newest-to-oldest frontier is **2019** (928 items / 20 tranches remaining). Only France's total target is numerically fixed; see the per-year class-A table in [worldcons-historical-backfill-target-plan-20260917.md](./worldcons-historical-backfill-target-plan-20260917.md).

M5-B5 subsequently resolved the 2017 QPC frontier blocker. The read-only official inventory cross-check showed that the official Conseil 2017 QPC annual/type facet publishes exactly **75** decisions while the approved DILA CONSTIT base stock plus all 21 ordered increments contain exactly **69** matching Conseil identities; the six missing decisions' Conseil record ids **and** official ECLIs are absent from every one of the 7,600 scanned stock/increment XML members. The official Conseil detail pages do not publish a JORF NOR, so the frozen official ECLI is the deterministic secondary absence cross-check. The six exact owner-approved 2017 QPC Conseil-provider omission tuples are `2016613QPC`, `2017663QPC`, `2017664QPC`, `2017665QPC`, `2017666QPC`, and `2017670QPC`. They are authorized by the additive immutable policy successor `france-dila-constit-2026-09-v3` and migration `20260917090000_constitutional_case_france_policy_v3_approval.sql`, which also extends the exact-tuple `source_inventory_item_upsert_v3` contract and the France attribution guard. v1/v2 stay immutable. A complete 2016-2010 read-only characterization found no further DILA omissions or duplicate identities. The 2013 QPC frontier then exposed one deterministic exact authority-URL case canonicalization: DILA `CONSTEXT000027147071` carries the uppercase Conseil record id `2012293_294_295_296QPC` in `URL_CC`, while the official site and the official 2013 QPC facet publish only the lowercase canonical `2012293_294_295_296qpc` (the uppercase form returns an HTTP 301 that the fail-closed governed fetch refuses to follow). This single exact tuple is authorized by the additive immutable successor `france-dila-constit-2026-09-v4` and migration `20260917120000_constitutional_case_france_policy_v4_approval.sql`. v1/v2/v3 stay immutable. A full 2010-2012 read-only redirect scan of all 312 official URLs found no other canonicalization.

**France Gate 5 QPC/DC historical completion (2026-09-17):** all **30 / 30** tranches (2010-2024 QPC and DC) and **1,280 / 1,280** items are `corpus_backfill_complete` in private shadow. Every authoritative tranche snapshot is `closed`, `authoritative_crosschecked`, has `expected_count = discovered_count`, and all of its items are `verified`, with zero active/expired claims, zero retry/terminal failures, zero nonterminal runs, and zero mismatches. The approved policy chain is `france-dila-constit-2026-09-v1` → `-v2` (2022 E1/E2) → `-v3` (six 2017 QPC E1 omissions) → `-v4` (one 2013 QPC E3 authority-URL canonicalization), with migrations `20260916090000`, `20260916100000`/`20260916101000`/`20260916102000`, `20260917090000`, and `20260917120000`. Two earlier sealed snapshots remain as immutable degraded predecessors that were replaced by a corrected snapshot under the successor policy: 2013 QPC `cb17e57e-407d-4953-92e7-822cf60f868b` (65/66, replaced by `12480df5-59a4-49bd-8c98-26a85001f6fb`) and 2012 QPC `429644d3-0294-4f81-9436-c9487a3d51b2` (72/74 transient fetch failures, replaced by `8ee417ec-8c85-4b93-96d3-49da7ea75ac2`). `public_enrichment_complete` remains **false**; Catalog publications, publication events, and Gemini calls remain **0**.
