...
evidence. Therefore `fulltext_rank_threshold_unagreed` is retired. `GO-SEARCH`
remains blocked pending deployed-Worker bearer-path runtime evidence and the
separate final M7 readiness decision. See
...
- [ ] Vectorize parity (M7.4-M7.6 foundation/canary work remains verified: isolated `worldcons-search-canary-v2` is at 100 vectors and the isolated D1 canary at 100 documents/FTS, with append-only 15->100 expansion and local-runtime + remote-bindings p50/p95 161/386 ms. The isolated `worldcons-search-canary` Worker is deployed but those latency figures are not a deployed-Worker SLO. M7.8-A has removed the former production semantic-oracle blocker: migration `20260926120000` is **APPLIED/VERIFIED** in the remote ledger, `public_article_projection_p3` now uses the provenance-locked artifact join + `coalesce` embedding authority, semantic audit is currentPublished=1258, projectionRows=1258, projectionEmbeddingNull=0, artifactBacked=1258, legacyOnly=872, all provenance mismatches=0, and semantic/hybrid smoke is 4/4 with `oracleDrift=0`. The rollout recovery is explicitly evidenced by read-only `--finalize-existing` without fabricating pre-apply digest equality. The independent generic lexical policy is now signed and passed; the sole remaining M7 operational evidence is the deployed-Worker bearer-path runtime run. No `GO-SEARCH`.)
