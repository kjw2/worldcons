...
function tableArgument(): ArticleRawRestoreTable {
  const value = argumentValue("table")?.trim() ?? "";
  if (!ARTICLE_RAW_RESTORE_TABLES.includes(value as ArticleRawRestoreTable)) {
    throw new Error("invalid_table");
  }
  return value as ArticleRawRestoreTable;
}
...
async function main() {
  ...
  // Executing puts the inline raw_text copy back, so it is gated on an explicit
  // acknowledgement and the article raw blob read flag before the Blob store is
  // created or any batch runs. A dry run needs neither the gate nor a Blob store:
  // it passes a null store, so it never creates one and never needs a Blob token.
  // WRITE is never required: this path only reads Blob objects and never writes or
  // deletes one.
  if (execute) {
    if (!flag("acknowledge-inline-restore")) {
      throw new Error("article_raw_restore.acknowledge_required");
    }
  ...
  output({
    event: "article_raw_restore_planned",
    articleTable,
    sourceKey,
    batchSize,
    maxBatches,
  ...
  while (batches < maxBatches) {
    const result = await runArticleRawRestoreBatch(
      ...
      { repository: postgresArticleRawRestoreRepository, store },
    );
    batches += 1;
    ...
    output({
      event: "article_raw_restore_batch",
      articleTable,
      sourceKey,
      batchNumber: batches,
      scanned: result.scanned,
      ...
      outcomes: result.outcomes.map(toSafeArticleRawRestoreOutcome),
      execute,
      blobObjectsWritten: 0,
      blobObjectsDeleted: 0,
      publicCatalogWrites: 0,
  ...
  output({
    event: execute ? "article_raw_restore_completed" : "article_raw_restore_dry_run_completed",
    articleTable,
    sourceKey,
    batches,
    scanned,
...
main().then((exitCode) => {
  ...
  output({
    event: "article_raw_restore_failed",
