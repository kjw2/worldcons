...
export function ArticleSourceSnapshot({ slug }: { slug: string }) {
  ...
  return (
    <details onToggle={handleToggle} className="border-y border-archive-line bg-archive-surface-soft px-1 py-4 sm:px-3">
      ...
      <div className="mt-4">
        ...
        {state.status === "loading" ? <div className="h-32 animate-pulse bg-white" aria-label="원문 스냅샷을 불러오는 중" /> : null}
        {state.status === "error" ? <p className="border-l-2 border-court bg-white p-4 text-sm leading-6 text-court">{state.error}</p> : null}
        {state.status === "loaded" ? (
          <pre className="max-h-96 overflow-auto whitespace-pre-wrap bg-white p-4 text-[13px] leading-7 text-ink-muted">{state.text}</pre>
        ) : null}
      </div>
    </details>
  );
