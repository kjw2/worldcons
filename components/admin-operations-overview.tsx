...
function Metric({ label, value, href, detail, tone = "neutral" }: { label: string; value: number; href: string; detail?: string; tone?: "neutral" | "warning" | "danger" }) {
  return (
    <Link href={href} className={`focus-ring block rounded-md border bg-white px-4 py-3 hover:bg-parchment ${tone === "danger" ? "border-court/30" : tone === "warning" ? "border-amber-300" : "border-rule"}`}>
      ...
      {detail ? <span className="mt-1 block text-xs leading-5 text-ink/48">{detail}</span> : null}
    </Link>
  );
}
...
export function AdminOperationsOverview({ snapshot }: { snapshot: AdminOperationsOverviewSnapshot }) {
  ...
  const candidateOther = Math.max(0, candidateAttention - work.counts.candidateOfficialDetail404 - work.counts.candidateDiscoveryEmpty);
    ...
    <div className="min-w-0 py-6">
      ...
      <section className="border-b border-rule bg-white" aria-labelledby="bpm-flow">
        ...
        <div className="grid lg:grid-cols-4">
          <Stage label="수집" detail="공식 원문 확인이 필요한 추적 후보" value={candidateAttention} href="/admin/candidates" icon={Database} />
          <Stage label="처리 / 요약" detail="기사 요약 대기 및 처리 상태" value={dashboard.totals.pendingSummaries} href="/admin/work?stage=process&attention=required" icon={Clock3} />
      ...
      <section className="border-b border-rule bg-parchment/35 px-4 py-5 sm:px-6" aria-labelledby="candidate-tracking">
        <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 id="candidate-tracking" className="text-base font-semibold text-ink">수집 추적 후보</h2>
            <p className="mt-1 max-w-3xl text-xs leading-5 text-ink/55">공식 원문을 아직 검증하지 못해 보관한 URL입니다. 기사 처리·요약 대기와 별도이며, 검증 전에는 요약하거나 공개하지 않습니다.</p>
          </div>
          <Link href="/admin/candidates" className="focus-ring inline-flex min-h-9 items-center gap-2 rounded-md border border-rule bg-white px-3 text-sm font-semibold text-ink/65 hover:bg-parchment">후보 목록<ArrowRight className="size-4" aria-hidden="true" /></Link>
        </div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <Metric label="전체 추적 후보" value={candidateAttention} href="/admin/candidates" detail="요약·공개 대상 아님" />
          <Metric label="공식 상세 404" value={work.counts.candidateOfficialDetail404} href="/admin/candidates?q=BVERFG_OFFICIAL_DETAIL_404" detail="공식 페이지 생성·경로 변경 확인 대기" tone="warning" />
          <Metric label="공식 탐색 결과 없음" value={work.counts.candidateDiscoveryEmpty} href="/admin/candidates?q=BVERFG_LIVE_DISCOVERY_EMPTY" detail="다음 수집에서 다시 탐색" tone="warning" />
          <Metric label="기타 추적 사유" value={candidateOther} href="/admin/candidates" detail="별도 확인이 필요한 후보" />
        </div>
      </section>

      <section className="px-4 py-5 sm:px-6" aria-labelledby="attention-signals">
        <div className="mb-3 flex items-start gap-2"><Radio className="mt-0.5 size-4 text-court" aria-hidden="true" /><div><h2 id="attention-signals" className="text-base font-semibold text-ink">운영 조치</h2><p className="mt-1 text-xs leading-5 text-ink/52">수집 추적 후보를 제외한 실제 실행·기사 처리·공개 업무입니다.</p></div></div>
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
          <Metric label="조치 필요" value={work.counts.backlog} href="/admin/work?scope=operations&attention=required" />
          <Metric label="기한 초과" value={work.counts.breached} href="/admin/work?scope=operations&attention=required&sla=breached&sort=sla" tone="danger" />
          <Metric label="실패" value={work.counts.failed} href="/admin/work?scope=operations&attention=required&state=failed" tone="danger" />
          <Metric label="장기 지연" value={work.counts.stale} href="/admin/work?scope=operations&attention=required&state=stale" tone="warning" />
          <Metric label="중단 요청" value={work.counts.abortRequested} href="/admin/work?scope=operations&type=execution&state=abort" tone="warning" />
          <Metric label="전달 대기" value={work.counts.outbox} href="/admin/work?scope=operations&type=outbox&attention=required" tone="warning" />
        </div>
      </section>
