...
export function evaluateP5RetirementReadiness(input: P5RetirementEvaluationInput): P5RetirementReport {
  ...
  const approvals = new Set(input.evidence.governance.approvedRoles);
  ...
  const gates: P5RetirementGate[] = [
    ...
    { key: "compatibility.zero", label: "No unexplained legacy reads or writes", passed: input.evidence.compatibility.unexplainedLegacyCount === 0, detail: `${input.evidence.compatibility.unexplainedLegacyCount} unexplained legacy observations.` },
    ...
    { key: "owners.approved", label: "All named owner roles approved", passed: P5_OWNER_ROLES.every((role) => approvals.has(role)), detail: `Approved roles: ${[...approvals].sort().join(", ") || "none"}.` },
  ...
  const evidenceDigest = crypto.createHash("sha256").update(payload).digest("hex");
  const signature = input.signingKey ? crypto.createHmac("sha256", input.signingKey).update(payload).digest("hex") : null;
